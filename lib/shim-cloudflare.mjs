...
import { UNDEF, isArr, isFn, isStr, hex, toStr, toUint } from '../util.mjs'
...
var header = (req, name) => req.headers?.get(name) || ''
, isGet = req => (req.method || 'GET') === 'GET'
...
, Cache = (map = new Map()) => ({
	match(req) {
		...
		if (item) {
			...
			else if (!item.vary || item.reqVary === varyKey(req, item.vary)) return item.res.clone()
		}
	},
	put(req, res) {
...
, evict = instances => {
	instances.forEach(({ db, instance }, key) => {
		if (instance.active) {
...
, idFromString = (hex, name) => ({
	equals: other => hex === '' + other,
	name: name || null,
	toString: () => hex
})
, newUniqueId = () => idFromString(hex(randomBytes(32)))
, durableAlarms = db => kvMap(db, 'do_alarms', ['time'])
, durableObject = (Cls, dir, env, DB, alarms = new Map(), init) => {
	var instances = new Map()
	, schedule = (id, key, time, retryCount = 0) => alarms.set(key, {
		time,
		timer: setTimeout(fireAlarm, Math.max(0, time - Date.now()), id, key, retryCount).unref?.()
	})
	, fireAlarm = async (id, key, retryCount) => {
		if (alarms.delete(key)) try {
			await get(id).alarm({ retryCount, isRetry: retryCount > 0 })
		} catch (e) {
			if (++retryCount <= 6) schedule(id, key, Date.now() + 2000 * (1 << (retryCount - 1)), retryCount)
		}
	}
	, get = id => {
		var key = '' + id, instance = (instances.get(key) || (instances.set(key, key = run(id, key)), key)).instance
	...
	, run = (id, key) => {
		...
		, kv = {
			...
			put: (key, value) => (heartbeat(), dbSet.run(key, JSON.stringify(value))),
			delete(key) {
				dbDel.run(key)
		...
		, ctx = {
			...
			storage: {
				...
				setAlarm: time => { heartbeat(), clearTimeout(alarms.get(key)?.timer), schedule(id, key, +time) },
				transactionSync,
				...
				sql: {
					exec(query, ...binds) {
						...
						, rows = stmt.all(...binds)
						, written = isWrite.test(query) ? changes.get().c : 0
						return {
		...
		, heartbeat = () => { if (instance) instance.active = true }
		, instance = new Cls(ctx, env)
	...
	Object.entries(descriptors).forEach(([name, descriptor]) => {
		...
		if (isFn(orig)) descriptor.value = name === 'fetch'
		? function(input, opts) {
			this.active = true
			return orig.call(this, input instanceof Request ? input : new Request(input, opts))
		}
		: function(...args) {
			this.active = true
	...
	for (var [key, entry] of alarms) schedule(idFromString(key), key, entry.time)
...
, KV = (db, name) => {
	...
	return {
		...
		async put(key, val, opts) {
			...
			if (val instanceof ReadableStream) val = new Response(val)
			dbSet.run(
				key,
				toUint(isFn(val.arrayBuffer) ? await val.arrayBuffer() : val),
...
, r2Wrap = (row, key) => {
	if (!row) return null
	var out = row.value != null ? new Response(row.value, { headers: { 'content-type': row.type } }) : {}
	out.key = key ?? row.name
	out.size = row.size
	out.etag = row.etag
	out.httpEtag = '"' + row.etag + '"'
	out.httpMetadata = { contentType: row.type }
	out.customMetadata = jsonParse(row.meta)
	out.uploaded = new Date(row.uploaded + 'Z')
	out.writeHttpMetadata = writeHttpMetadata
	return out
}
, checkCond = (cond, row, val, h) => !cond || !row || !(
	...
	(row = new Date(row.uploaded + 'Z'), val = h ? h.get('if-unmodified-since') : cond.uploadedBefore) && row >= new Date(val) ||
...
, R2 = (db, opts) => {
	...
	, { dbDel, dbGet, dbInfo, dbList, dbSet } = kvTable(db, {
		name,
		extra: ',size INTEGER,etag TEXT,type TEXT,meta TEXT,uploaded TEXT',
		fresh: ttl ? ' AND uploaded>datetime(\'now\', \'-' + (ttl | 0) + ' seconds\')' : '',
		wrap: r2Wrap,
	})
	return {
		get(key, opts) {
		...
		async put(key, val, opts) {
			if (val instanceof ReadableStream) val = new Response(val)
			if (isFn(val?.arrayBuffer)) val = await val.arrayBuffer()
			if (opts?.onlyIf && !checkCond(opts.onlyIf, dbInfo(key))) return null
			var buf = toUint(val ?? new Uint8Array(0))
			, etag = createHash('md5').update(buf).digest('hex')
			, type = opts?.httpMetadata?.contentType || opts?.contentType || 'application/octet-stream'
			, custom = opts?.customMetadata || {}
			, uploaded = new Date().toISOString().slice(0, 19).replace('T', ' ')
			dbSet.run(key, buf, buf.length, etag, type, JSON.stringify(custom), uploaded)
			return { key, size: buf.length, etag, httpEtag: '"' + etag + '"', httpMetadata: { contentType: type }, customMetadata: custom, uploaded: new Date(), writeHttpMetadata }
		},
		delete: keys => {
...
, parseCron = expr => {
	...
	expr.trim().split(/\s+/).forEach((field, i) => {
		for (var part of field.split(',')) {
			var [range, step] = part.split('/')
			, [lo, hi] = range === '*' ? [i < 2 ? 0 : 1, max[i]] : range.split('-').map(Number)
			, v = lo
