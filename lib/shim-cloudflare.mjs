...
import { UNDEF, fail, isArr, isFn, isStr, header, hex, rand, toStr } from '../util.mjs'
...
, evict = instances => {
	instances.forEach(entry => {
		if (entry.active || entry.ctx.getWebSockets().length) entry.active = false
		else entry.close()
	})
}
...
, newUniqueId = () => idFromString(hex(rand(32)))
...
, durableObject = (Cls, dir, env, alarms = new Map()) => {
	...
	, run = (id, key) => {
		...
		, ctx = {
			...
			getTags: ws => sockets.get(ws) || fail('Not accepted WebSocket'),
			getWebSockets: tag => [...sockets.keys()].filter(ws => ws.readyState === 3 ? !sockets.delete(ws) : tag == UNDEF || sockets.get(ws).includes(tag)),
